...
const NoteCard: React.FC<NoteCardProps> = ({
  ...
  const getLeftBorder = () => {
    if (note.isTrashed) return "border-l-red-400";
    if (note.isArchived) return "border-l-amber-400";
    if (note.isTodo) return "border-l-blue-400";
    return "border-l-emerald-400";
  };

  return (
    <div
      className={`group relative rounded-xl border-2 border-l-4 p-4 transition-all duration-300 ease-in-out cursor-pointer
      ...
      <div className="flex-1 mb-4 overflow-hidden">
        {note.isTodo && note.todos && note.todos.length > 0 ? (
          <div className="space-y-2 h-full flex flex-col">
            {/* Progress Bar */}
            {total > 0 && (
              <div className="w-full bg-slate-200 rounded-full h-2 mb-3 flex-shrink-0">
                <div
          ...
          <div className="h-full flex flex-col">
            <p
              className={`text-slate-600 text-sm leading-relaxed flex-1 ${
                isListView ? "line-clamp-3" : "line-clamp-6"
              } overflow-hidden`}
            >
        ...
        <div className="flex items-center gap-2">
          ...
          {showArchiveButton && onArchive && (
            <button
              ...
              className="group relative p-2.5 rounded-xl bg-white border-2 border-slate-200 hover:bg-blue-50 
          ...
          hover:shadow-lg shadow-sm active:shadow-inner btn-click-animate"
              title="Archive Note"
            >
              <RiInboxArchiveLine className="w-4 h-4 transition-transform duration-150 group-active:scale-110" />
            </button>
          )}
          ...
          {showTrashButton && onTrash && (
            <button
              ...
              className="group relative p-2.5 rounded-xl bg-white border-2 border-slate-200 hover:bg-orange-50 
          hover:border-orange-300 text-slate-600 hover:text-orange-600 transition-all duration-200 
          hover:shadow-lg shadow-sm active:shadow-inner btn-click-animate"
              title="Move to Trash"
            >
              <GoTrash className="w-4 h-4 transition-transform duration-150 group-active:scale-110" />
            </button>
          )}
          ...
          {showDeleteButton && onDelete && (
            <button
              ...
              className="group relative p-2.5 rounded-xl bg-white border-2 border-red-200 hover:bg-red-50 
          hover:border-red-300 text-red-500 hover:text-red-600 transition-all duration-200 
          hover:shadow-lg shadow-sm active:shadow-inner btn-click-animate"
              title="Delete Forever"
            >
              <AiFillDelete className="w-4 h-4 transition-transform duration-150 group-active:scale-110" />
            </button>
          )}
        </div>
        ...
        <div>
          ...
          {showRestoreButton && onRestore && (
            <button
              ...
              className="group relative flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 
          text-white text-sm font-medium rounded-xl hover:from-emerald-600 hover:to-emerald-700 
          transition-all duration-200 hover:shadow-xl shadow-lg active:shadow-inner btn-click-animate"
              title="Restore to Home"
            >
              <BiHome className="w-4 h-4 transition-transform duration-150 group-active:scale-110" />
